...
bot.hears("📞 Консультація", async (ctx) => {
    ...
    } else if (currentMenu === "flatris") {
        await processServiceRequest(ctx, "Інтеграція з Flatris (шахматка)");
    }
});
...
const crmMenu = Markup.keyboard([
    ...
    ["📞 Інтеграція з RingoStat", "🧩 Інтеграція з Flatris (шахматка)", "🔙 На головну"]
]).resize().oneTime();
...
bot.hears("🧩 Інтеграція з Flatris (шахматка)", async (ctx) => {
    currentMenu = "flatris";
    await ctx.reply(
        "🧩 Інтеграція Flatris (шахматка бронювання):\n\n" +
        "Інтерактивна шахматка для ЖК / котеджного містечка\n\n" +
        "Показ реального статусу: вільно / заброньовано / продано\n" +
        "Синхронізація з CRM: заявки, статуси, менеджери\n" +
        "Автоматичне оновлення даних на сайті\n" +
        "Можливість додати фото, планування, метраж, ціну\n" +
        "Відображення воронки продажів прямо в інтерфейсі\n" +
        "Працює з Kommo, Tilda, Webflow, Wordpress\n\n" +
        "💰 Вартість: від $1,000 (разовий платіж)\n" +
        "📌 Ціна залежить від кількості об’єктів та інтеграцій",
        requestServiceMenu
    );
});
